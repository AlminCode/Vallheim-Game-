Shader "RenderFX/SkyboxHDR" {
Properties {
	_Tint ("Tint Color", Color) = (.5, .5, .5, .5)
	_Multiplier("Multiplier", Float ) = 10.0
...
SubShader {
	...
	float4 skybox_frag (v2f i, sampler2D smp)
	{
		fixed4 tex = tex2D (smp, i.texcoord);
		float4 col;
		
		col.rgb = tex.rgb - unity_ColorSpaceGrey;
